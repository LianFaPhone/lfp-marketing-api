...
func InitDbTable() {
	...
	err = db.GDbMgr.Get().Set("gorm:table_options", "ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 AUTO_INCREMENT=1;").AutoMigrate(&IdRecorder{}, &ActiveCode{}, &BlacklistIdcard{}, &CardOrderUrl{}, &AdTrack{}).Error
	if err != nil {
		log.ZapLog().Error("AutoMigrate err", zap.Error(err))
	}
	...
	err = db.GDbMgr.Get().Set("gorm:table_options", "ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 AUTO_INCREMENT=1;").AutoMigrate(&CardOrderRetry{}, &BsProvice{}, &BsCity{}, &BsArea{}, &PdPartner{},&DxnbhkProvice{},&DxnbhkCity{},&DxnbhkArea{}).Error
	if err != nil {
		log.ZapLog().Error("AutoMigrate err", zap.Error(err))
	}
