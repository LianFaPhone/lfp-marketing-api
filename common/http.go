...
func HttpSend3(url string, body io.Reader, method string, headers map[string]string, ckArr []*http.Cookie) ([]byte, error) {
	if len(method) == 0 {
		method = "GET"
	}
	req, err := http.NewRequest(method, url, body)
	...
	for i:=0; i < len(ckArr); i++ {
		req.AddCookie(ckArr[i])
	}
